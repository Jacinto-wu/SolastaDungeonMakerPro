...
namespace SolastaDungeonMakerPro.Patches.DungeonEditor
{
    ...
    {
        // adjusts the dungeon editor details panel to take half the screen height
        [HarmonyPatch(typeof(UserLocationEditorScreen), "OnBeginShow")]
        internal static class UserLocationEditorScreen_Bind_Patch
        {
            internal static void Postfix(UserLocationEditorScreen __instance, List<GuiPanel> ___subPanels)
            {
                // starts all panels folded
                foreach (var subPanel in ___subPanels)
                {
                    (subPanel as BaseBlueprintSelectionPanel).FoldAll();
                }

                // adds more real state to property panels
                var x = __instance.ControlPanel.RectTransform.sizeDelta.x;
                var y = __instance.ControlPanel.RectTransform.sizeDelta.y;

                __instance.SelectedGadgetPanel.RectTransform.sizeDelta = new UnityEngine.Vector2(x, y * 1f / 2f);
            }
        }
    }

    //
    // @IMPPHIL: is this still required?
    //
